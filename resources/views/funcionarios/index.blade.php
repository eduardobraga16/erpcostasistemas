...
<table class="table">
  ...
  <tbody>
  	...
  	@foreach($funcionarios as $key)
	      ...
	      <th>{{$key['id']}}</th>
	      <td>{{$key['nome']}}</td>
	      <td><?php if($key['administrador'] == 's'){echo "Sim";}else{echo "Não";} ?></td>
        <td>{{$key['nome_estabelecimento_join']}}</td>
        <td class="btns-crud">
          <button class="btn btn-success"><a class="text-white" href="{{url('funcionarios')}}/{{$key['id']}}/edit">Editar</a></button>
          <button class="btn btn-danger btn-excluir"><a class="text-white" href="{{url('funcionarios/excluir')}}/{{$key['id']}}">Excluir</a></button>
        </td>
	    </tr>
  	@endforeach
