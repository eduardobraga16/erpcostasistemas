...
<?php if($venda['id_status'] == '2' || $venda['id_status'] == '3' || $venda['id_status'] == '4'){redirect('vendas')->send();} ?>

<h1>PDV - Atender Venda Nº {{$venda['id']}}</h1>
...
<form class="row g-3" id="formulario-front-cad-prod" method="post" action="{{url('vendas/finalizarvenda')}}/{{$venda['id']}}" enctype="multipart/form-data">
  ...
  <div class="col-md-6 box-carrinho-pai">
    ...
    <label for="troco" class="form-label"><input type="hidden" value="0" id="troco" name="troco">Troco: <span class="str-color1">R$ <span class="troco-str">{{moedaBr($venda['troco'])}}</span></span></label><br/>
      ...
      <li class="nav-item">
        <select name="forma_pagamento" id="forma_pagamento" required="true" class="form-select">
          @foreach($formaspagamento as $forma)
            <option value="{{$forma['id']}}" <?php if($forma['forme'] == "avista"){echo "selected";} ?> >{{$forma['forma']}}</option>
          @endforeach
        </select>
      </li>
    ...
    <br/>
    <input type="text" class="form-control" id="nome_cliente" value="{{$venda['nome_cliente']}}" name="nome_cliente" placeholder="Digite o nome do cliente" required="true">
