...
<form class="row g-3" id="formulario-front-cad-prod" method="post" action="{{url('vendas/finalizarvenda')}}/{{$venda['id']}}" enctype="multipart/form-data">
  ...
  <label for="preco" class="form-label">Total: <span class="str-color1">R$ <span class="total-str">{{moedaBr($venda['total'])}}</span></span></label><br/>
    <label for="troco" class="form-label"><input type="hidden" value="0" id="troco" name="troco">Troco: <span class="str-color1">R$ <span class="troco-str">0</span></span></label><br/>
    ...
    <br/>
    <input type="text" class="form-control" id="nome_cliente" value="{{$venda['nome_cliente']}}" name="nome_cliente" autocomplete="off" placeholder="Digite o nome do cliente" required="true">
